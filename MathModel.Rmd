...
library(knitr)
library(kableExtra)
Tab <- read.csv("data/Model_assumptions.csv") %>% as.data.frame() %>% dplyr::rename("Comment"=Model.Limitation,"Effect of Addressing Comment"=Effect.of.Addressing.Limitation)
Tab$Comment <- Tab$Comment %>% as.character()
#knitr::kable(Tab,align = "c")
Tab[1,2] <- gsub("_"," →",Tab[1,2])
Tab %>% 
  kable('html',align = "c") %>% 
